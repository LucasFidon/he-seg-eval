...
from definitions import SPACING
...
def _binarize(seg, fg_class):
    ...
    :param fg_class: int or int list; classes in seg corresponding to the foreground.
...
def mean_dice_score(seg_1, seg_2, labels_list):
    """
    Compute the mean of the Dice scores for the labels in labels_list
    between the segmentations seg_1 and seg_2.
    :param seg_1: numpy int array.
    :param seg_2: numpy int array.
    :param labels_list: int list. labels to use to compute the mean
    :return:
    """
...
def haussdorff_distance(mask_gt, mask_pred, fg_class,
                        percentile=100, spacing_mm=SPACING):
    assert np.all(mask_gt.shape == mask_pred.shape), 'Shapes of ground truth and prediction are not the same.'

    bin_mask_gt = np.squeeze(_binarize(mask_gt, fg_class=fg_class))
    bin_mask_pred = np.squeeze(_binarize(mask_pred, fg_class=fg_class))

    # Compatibility for 2D
    if bin_mask_gt.ndim == 2:
        bin_mask_gt = np.expand_dims(bin_mask_gt, axis=2)
        bin_mask_pred = np.expand_dims(bin_mask_pred, axis=2)
